...
version: 1.1.0
author: Codex (adapted from obra/superpowers)
license: MIT
...
The feedback loop is the debugging work. Before reading code to build a theory, create or identify a **tight** command that can go red on the user's exact symptom and green when the bug is fixed. A tight loop is fast, deterministic, agent-runnable, and specific enough to catch this bug 鈥?not merely "doesn't crash".
...
**Ways to construct a loop 鈥?try in roughly this order:**
...
**WHEN system has multiple components (API 鈫?service 鈫?database, CI 鈫?build 鈫?deploy):**
...
- Generate 3鈥? plausible hypotheses before testing any single one.
...
- Did it work? 鈫?Phase 4
- Didn't work? 鈫?Form NEW hypothesis
...
# Run full suite 鈥?no regressions
pytest tests/ -q
```

### 4. If Fix Doesn't Work 鈥?The Rule of Three
...
This is NOT a failed hypothesis 鈥?this is a wrong architecture.

---

## Red Flags 鈥?STOP and Follow Process
...
- **`search_files`** 鈥?Find error strings, trace function calls, locate patterns
- **`read_file`** 鈥?Read source code with line numbers for precise analysis
- **`terminal`** 鈥?Run tests, check git history, reproduce bugs
- **`web_search`/`web_extract`** 鈥?Research error messages, library docs
...
delegate_task(
    ...
    4. Report findings 鈥?do NOT fix yet
